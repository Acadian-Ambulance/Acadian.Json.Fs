...
type Item = { Id: int; Desc: string option }
...
let tests =
    testList
        "Tests"
        [ testList
              "JsonConverters.OptionConverter"
              [ test "Test converting a list of items with optional members to json" {
                  let items =
                      [ { Id = 1; Desc = None }
                        { Id = 2; Desc = Some "stuff" } ]

                  let jsons =
                      JsonConvert.SerializeObject(items, OptionConverter())

                  jsons
                  |> Expect.equal "" "[{\"Id\":1,\"Desc\":null},{\"Id\":2,\"Desc\":\"stuff\"}]"
                }
                test "Test converting an item with None desc json" {
                    let item = { Id = 1; Desc = None }

                    let json =
                        JsonConvert.SerializeObject(item, OptionConverter())
                ...
                test "Test converting a list of optional items with optional members to json" {
                    ...
                    jsons
                    |> Expect.equal "" "[{\"Id\":1,\"Desc\":null},{\"Id\":2,\"Desc\":\"stuff\"},null]"
                } ] ]
